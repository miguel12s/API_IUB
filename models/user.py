...
class ModelUser:
    ...
    def obtenerHorarioEstudiante(id_estudiante:int,data):
        try:
    ...
    def obtenerTutoriasPendientes(user_id:int):
     try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
SELECT ht.id_tutoria,f.facultad,p.programa,m.materia ,s.salon,le.id_usuario,txe.estado,ht.cupos,ht.tema,ht.fecha,date_format(ht.hora_inicial,'%H:%i') as hora_inicial,date_format(ht.hora_final,'%H:%i') as hora_final,u.nombres,u.apellidos  FROM `horario_tutorias` ht 
...
""",(user_id,))
    ...
    def obtenerTutoriaFinalizada(user_id:int):
     try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
            SELECT ht.id_tutoria,f.facultad,p.programa,m.materia ,s.salon,le.id_usuario,txe.estado,ht.cupos,ht.tema,ht.fecha, date_format(ht.hora_inicial,'%H:%m') as hora_inicial,date_format(ht.hora_final,'%H:%i') as hora_final,u.nombres,u.apellidos,le.comentario  FROM `horario_tutorias` ht 
            join lista_estudiantes le on le.id_tutoria=ht.id_tutoria
            join salones s on ht.id_salon=s.id_salon
            join tipoxestado txe on ht.id_estado_tutoria=txe.id_tipoxestado 
            join fpxmateria fpxm on fpxm.id_fpxm=ht.id_fpxm
    ...
    def obtenerTutoriaFinalizadaDocente(user_id:int):
     try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
             SELECT ht.id_tutoria,f.facultad,p.programa,m.materia ,s.salon,ht.id_usuario,txe.estado,ht.cupos,ht.tema,ht.fecha, date_format(ht.hora_inicial,'%H:%i') as hora_inicial,date_format(ht.hora_final,'%H:%i'),u.nombres,u.apellidos  FROM `horario_tutorias` ht 
            join salones s on ht.id_salon=s.id_salon
            join tipoxestado txe on ht.id_estado_tutoria=txe.id_tipoxestado 
            join fpxmateria fpxm on fpxm.id_fpxm=ht.id_fpxm
            join facultadxprograma fxp on fxp.id_fxp=fpxm.id_fxp
...
""", (id_tutoria,))
    ...
    def verify_hour(id_tutoria,id_user):
        bd=get_db_connection()
        cursor=bd.cursor()
        sql="select ht.fecha,date_format(ht.hora_inicial,'%H:%m') as hora_inicial,date_format(ht.hora_final,'%H:%m') as hora_final from horario_tutorias ht where ht.id_tutoria=%s"
        cursor.execute(sql,(id_tutoria,))
        result=cursor.fetchone()
        sql="""select count(*) from lista_estudiantes le
join horario_tutorias ht on ht.id_tutoria=le.id_tutoria
join usuarios u on u.id_usuario=le.id_usuario
where ht.fecha=%s and ht.hora_inicial>=%s and ht.hora_final<=%s and le.id_usuario=%s"""
        cursor.execute(sql,(result[0],result[1],result[2],id_user))
