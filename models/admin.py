...
where f.id_facultad=(select f2.id_facultad from facultades f2 where  f2.facultad=%s ) and p.id_programa=(select p2.id_programa from programas p2 where p2.programa=%s) """, (user['facultad'], user['programa']))
                    ...
                    cursor.execute(
                        'INSERT INTO `fpxusuario`(`id_fxp`, `id_usuario`) VALUES (%s,%s)', (id_fxp, id_user))
...
""", (id_fxp, id_user))
    ...
    def getMateriaForIds(materia: MateriaIds):
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            payload=[]
            cursor.execute(
                """
select m.materia,m.id_materia from usuarios u 
join fpxmateria fpxm on fpxm.id_fxp=(select id_fxp from facultadxprograma fxp where fxp.id_facultad=%s and fxp.id_programa=%s)
join materias m on m.id_materia=fpxm.id_materia
where u.id_usuario=%s
                """,(materia.id_facultad,materia.id_programa,materia.id_docente))
            result = cursor.fetchall()
            for i in result:
                content={
                    "materia":i[0],
            ...
            if result:
                ...
                raise HTTPException(
                    status_code=404, detail="no existen materias asociadas con esa facultad y programa")
