...
from schemas.Peticion import Peticion
...
@horario.get('/horario-estudiante')

def obtenerHorarioEstudiante(request:Request):
     try:
           headers=request.headers
           payload=Security.verify_token(headers)
           user_id=payload['id_usuario']
           rpta=nueva_horario.obtenerHorarioEstudiante(user_id)
           return rpta
     except Exception as e:
          raise HTTPException(status_code=400,detail=e)

@horario.get('/horario-docente')

def obtenerHorarioDocente(request:Request):
     try:
           headers=request.headers
           payload=Security.verify_token(headers)
           user_id=payload['id_usuario']
           rpta=nueva_horario.obtenerHorarioDocente(user_id)
           return rpta
     except Exception as e:
          raise HTTPException(status_code=400,detail=e)
          
@horario.post('/create_notificacion_horario')

def createNotificacionHorario(solicitud:Solicitud,request:Request):
     
     try:
           headers=request.headers
           payload=Security.verify_token(headers)
           user_id=payload['id_usuario']
           rpta=nueva_horario.createNotificacionHorario(user_id,solicitud)
           return rpta
     except Exception as e:
          raise HTTPException(status_code=400,detail=e)
     
@horario.get('/mostrar-solicitud-horario')

def mostrarSolicitudHorario(request:Request):
     try:
           headers=request.headers
           payload=Security.verify_token(headers)
           user_id=payload['id_usuario']
           rpta=nueva_horario.mostrarSolicitudHorario(user_id)
           return rpta
     except Exception as e:
          raise  HTTPException(status_code=400,detail=e)
     
@horario.get('/mostrar-solicitudes')

def mostrarSolicitudes(request:Request):
      try:
           headers=request.headers
           payload=Security.verify_token(headers)
           user_id=payload['id_usuario']
           rpta=nueva_horario.mostrarSolicitudHorarioDocente(user_id)
           return rpta
      except Exception as e:
          raise  HTTPException(status_code=400,detail=e)       

@horario.put('/cambiar-estado-peticion')

def cambiarEstadoPeticion(request:Request,peticion:Peticion):
     try:
           headers=request.headers
           payload=Security.verify_token(headers)
           user_id=payload['id_usuario']
           rpta=nueva_horario.cambiarEstadoPeticion(peticion.id,user_id)
           return rpta
     except Exception as e:
          raise HTTPException(status_code=400,detail=e)

@horario.post('/peticion-aceptada')

def peticionAceptada(request:Request,solicitud:PeticionAceptada):
     try:
           headers=request.headers
           payload=Security.verify_token(headers)
           user_id=payload['id_usuario']
           rpta=nueva_horario.peticionAceptada(solicitud,user_id)
           return rpta
     except Exception as e:
        raise HTTPException(status_code=400,detail=e)
