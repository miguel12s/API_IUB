...
from models.Horario import SchedulerCalendar
...
class HorarioController:
    def getHorario(self,id):
        try:
            ...
            cursor.execute("""
              SELECT ht.id_tutoria,f.facultad,p.programa,m.materia ,s.salon,ht.id_usuario,txe.estado,ht.cupos,ht.tema,ht.fecha, date_format(ht.hora_inicial,'%H:%i') as hora_inicial,date_format(ht.hora_final,'%H:%i'),se.sede  FROM `horario_tutorias` ht 
...
""", (horario.cupos, horario.tema, horario.hora_inicial, horario.hora_final, horario.fecha, id))
    ...
    def getHorarioForIdUsuario(self,id:int):
        try:
            rpta=SchedulerCalendar.getHorarioForIdEstudiante(id)
            return rpta

        except mysql.connector.Error as err:
            return HTTPException(status_code=404,detail='no existen horarios')
        
    def createNotificacionHorario(self,id_user,solicitud:Solicitud):
        try:
             rpta=SchedulerCalendar.createNotificacionHorario(solicitud,id_user)
             return rpta
        except mysql.connector.Error as err:
            return HTTPException(status_code=404,detail='no existen horarios')
    ...
    def agendarTutoria(self,id_tutoria:int,user_id:int):
        try:
            if(ModelUser.verificar_agendamiento(id_tutoria,user_id)==0):
              isPermit=ModelUser.verify_hour(id_tutoria,user_id)
              print('isPermit',isPermit)
              if(isPermit==0):
                rpta=ModelUser.agendar_tutoria(id_tutoria,user_id)
                ModelUser.actualizarCupos(id_tutoria)
                return rpta
              return {"error":"hay un cruce de horarios"}
    ...
    def obtenerHorarioEstudiante(self,id_usuario):
        try:
            rpta=SchedulerCalendar.getSchedulerCalendar(id_usuario)
            return rpta
        except Exception as e:
            raise HTTPException(status_code=400,detail=e)
    def obtenerHorarioDocente(self,id_usuario):
        try:
            rpta=SchedulerCalendar.getSchedulerCalendarTeacher(id_usuario)
            return rpta
        except Exception as e:
            raise HTTPException(status_code=400,detail=e)
    def mostrarSolicitudHorario(self,id_usuario):
        try:
            rpta=SchedulerCalendar.mostrarSolicitudHorario(id_usuario)
            return rpta
        except Exception as e:
            raise HTTPException(status_code=400,detail=e)
        
    def mostrarSolicitudHorarioDocente(self,id_usuario):
        try:
            rpta=SchedulerCalendar.mostrarSolicitudes(id_usuario)
            return rpta
        except Exception as e:
            raise HTTPException(status_code=400,detail=e)
    def cambiarEstadoPeticion(self,id_peticion,id_user):
        try:
            rpta=SchedulerCalendar.cambiarEstadoPeticion(id_peticion,id_user,25)
            return rpta
        except Exception as e:
            raise HTTPException(status_code=400,detail=e)
    def peticionAceptada(self,solicitud:PeticionAceptada,id_user):
        try:
            rpta=SchedulerCalendar.peticionAceptada(solicitud,id_user)
            return rpta
        except Exception as e:
            raise HTTPException(status_code=400,detail=e)
